...
filter.colors <- c(
  "yellow", "yellowgreen", "orange", "red",          "red3",     "blue",
  "purple", "green",       "green3", "deepskyblue3", "skyblue",  "black"
)
```
...
filter.labels <- c(
  "#8",   "#11",  "#21", "#23A", "#25A", "#38A",
  "#47",  "#56",  "#58", "#80A", "#82A", "None"
)
```

```{r jupiter.setup, echo=F}
...
preprocess.filter <- function(df, wavelength, name) {
  data.frame(
    ...
    transmission = approx(df$wavelength, df$transmission, wavelength)$y, 
    name=name
  )
}
```
...
p <- ggplot(data=vision) +
  ...
  labs(title="Photopic/Scotopic Vision", 
       x="Wavelength [nm]", 
...
sun.visual$vision <- factor(sun.visual$vision, levels=c("Scotopic","Photopic")) # Reorder vision.
```

Flux for visual observing of sunlit object
```{r}
sun.flux <- sun.visual %>%
...
Plot flux for visual observing of sunlit object
```{r}
p <- ggplot(sun.flux) +
  ...
  facet_grid(. ~ vision) +
  labs(title="Visual Observing of Sunlit Object",
       x="Wavelength [nm]",
       y="Flux [lm/W]") +
  theme_bw()
p

if (write.plot.enabled) write.plot(p, "Visual Observing of Sunlit Object.png")
```
...
if (write.plot.enabled) write.plot(p, "Jupiter NEB and NTrZ Reflectance.png")
```
...
Flux and contrast for visual observing Jupiter
```{r}
...
Plot relativ flux and contrast for visual observing Jupiter 
```{r}
...
Function to set up data frame for visula observing Jupiter
```{r jupiter.setup, echo=T}
```
...
Function to set up data frame for visual observing of sunlight
```{r sun.setup, echo=T}
```
