...
filter <- rbind(filter, read.filter("Meade 4000 Filter 82.csv","82"))
```

Make a fix plot order using increasing fiter number
```{r}
filter$code <- factor(filter$code, levels=c("8",   "11",  "21", 
                                            "23A", "25A", "38A", 
                                            "47",  "56",  "58", 
                                            "80A", "82"))
```
...
```{r}
write.plot <- function(p, filename, width=1024, height=576) {
  png(filename,width, height)
  print(p + theme_bw(base_size=16))
  dev.off()
}
...
if (write.plot.enabled) write.plot(p, "Photopic and Scotopic Vision with Meade 4000 Filter.png")
```
